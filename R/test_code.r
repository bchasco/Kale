...
rtmb_model <- function(parms){
    ...
    C_t <- rep(0,max(t_k))      # Total carcasses over time
    T_t <- rep(0,max(t_k))      # Total carcasses over time
    ...
    for (t in 2:max(t_k)) {
      ...
      T_available[t] = exp(B[t] + par_lambda) * plogis(psi_par) + T_available[t-1] * plogis(phi_par);
    }
    ...
    for (t in unique(t_k)) {
      ...
      if (t > 1) {
        for (r in min((t + 1),16):(max(t_k)-1)) {  # Future time steps

          E_R[t, r] <- T_available[t] * exp(phi_par)^(r - t)
          R_t[t,r] <- sum(na.omit(n[r_k == r & t_k == t & tag == 1]))  # Recaptured tagged carcasses
          E_Rp[t, r] <- E_R[t,r] * plogis(p_par)
          # Likelihood for observed recaptures
          nll_recapture[t] <- nll_recapture[t] +  RTMB::dnorm(R_t[t,r], E_Rp[t, r], exp(R_sig), log = TRUE)

        }

        # #All the fish that were never observed
        tmp_Rt <- sum(na.omit(n[is.na(r_k) & t_k == t & tag == 1]))
        E_R_tmp <- T_available[t] * (1 - sum(exp(phi_par)^(t:16 - (t-1))))
        nll_recapture[t] <- nll_recapture[t] + RTMB::dnorm(tmp_Rt, E_R_tmp * (1-plogis(p_par)), exp(R_sig), log = TRUE)
      }
    }
